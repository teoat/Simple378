...
async def submit_decision(
    ...
    result = await db.execute(
        ...
        .where(models.AnalysisResult.id == analysis_uuid)
        .with_for_update()
    )
...
async def download_case_report(
    ...
    return StreamingResponse(
        ...
        headers={
            "Content-Disposition": f"attachment; filename=case_report_{analysis_id}.pdf"
        },
    )
        ...
        history.append(
            {
                ...
                "reviewer_id": (
                    str(analysis.reviewer_id) if analysis.reviewer_id else None
                ),
                "created_at": (
