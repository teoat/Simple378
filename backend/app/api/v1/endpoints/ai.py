...
@limiter.limit(
    "10/hour"
)  # Rate limit: 10 AI investigations per hour (expensive operation)
async def investigate_subject(
    ...
    try:
        # Initialize state
        initial_state = {"subject_id": str(subject_id), "messages": [], "findings": {}}
        ...
        raise HTTPException(
            status_code=500, detail=f"AI Investigation failed: {str(e)}"
        )
...
async def chat_with_ai(
    ...
    try:
        ...
        if case_id:
            try:
                ...
                if subject:
                    ...
                    case_context = {
                        "subject_name": (
                            subject.encrypted_pii.get("name", f"Subject {case_id[:8]}")
                            if subject.encrypted_pii
                            else f"Subject {case_id[:8]}"
                        ),
                        "risk_score": analysis.risk_score if analysis else 0,
                        "status": (
                            analysis.adjudication_status if analysis else "unknown"
                        ),
        ...
        response_data = await llm_service.generate_chat_response(
            message=message, persona=persona, case_context=case_context
        )
...
async def proactive_suggestions(
    ...
    try:
            ...
            suggestions = [
                {
                    ...
                    "priority": "urgent",
                    "actions": [
                        {"label": "View Alerts", "action": "navigate_to_adjudication"}
                    ],
                },
                {
                    "type": "opportunity",
                ...
                },
                {
                    ...
                    "priority": "high",
                    "actions": [
                        {"label": "Investigate Spike", "action": "view_analytics"}
                    ],
                },
            ]
            ...
            suggestions = [
                {
                    ...
                    "priority": "low",
                    "actions": [{"label": "View Cases", "action": "navigate_to_cases"}],
                }
            ]
        ...
        raise HTTPException(
            status_code=500, detail=f"Proactive suggestions failed: {str(e)}"
        )
...
"""
        ...
        for persona in personas:
            ...
            verdict = "suspicious"
            if (
                "fraud" in response_text.lower()
                or "fraudulent" in response_text.lower()
            ):
                verdict = "fraud_likely"
            elif (
                "innocent" in response_text.lower()
                or "legitimate" in response_text.lower()
            ):
                verdict = "innocent"
        ...
        raise HTTPException(
            status_code=500, detail=f"Multi-persona analysis failed: {str(e)}"
        )
...
"""
        ...
        return {
            ...
            "keyFindings": [
                "Transaction volume within normal ranges",
...
async def entity_disambiguation(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Entity disambiguation failed: {str(e)}"
        )
