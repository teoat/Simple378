...
class LLMService:
    ...
    async def generate_response(
        self, messages: List[BaseMessage], system_prompt: Optional[str] = None
    ) -> str:
        """
    ...
    async def route_decision(
        self, messages: List[BaseMessage], available_agents: List[str]
    ) -> Dict[str, Any]:
        """
...
Use the route_to_agent function to make your decision."""
        ...
        messages_with_system = [SystemMessage(content=supervisor_prompt)] + list(
            messages
        )
        ...
        for block in response.content:
            if (
                ...
                and block.get("name") == "route_to_agent"
            ):
                return block["input"]
    ...
    async def generate_chat_response(
        self, message: str, persona: str, case_context: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Any]:
        """
            ...
            if "interview" in message_lower or "interrogation" in message_lower:
                suggestions.extend(
                    [
                        ...
                        "Review Suspect Background",
                        "Plan Interrogation Strategy",
                    ]
                )
