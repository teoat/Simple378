...
class AutomatedInvestigationWorkflow:
    ...
    async def continue_investigation(
        self, execution_id: str, db: AsyncSession
    ) -> Dict[str, Any]:
        """
        ...
        return {"status": "continued", "message": "Investigation workflow continued"}

    async def execute_workflow_step(
        self, subject_id: str, step: InvestigationStep, db: AsyncSession
    ) -> Dict[str, Any]:
        """
...
class WorkflowStep:
    ...
    async def execute(
        ...
        try:
            ...
            return {"status": "failed", "step": self.step_type.value, "error": str(e)}
...
class WorkflowExecution:
    ...
    def __init__(
        ...
        self.id = (
            f"execution_{subject_id}_{datetime.utcnow().strftime('%Y%m%d_%H%M%S')}"
        )
    ...
    async def execute_step(
        self, step: InvestigationStep, db: AsyncSession
    ) -> Dict[str, Any]:
        """Execute a specific workflow step."""
    ...
    def get_next_steps(self) -> List[InvestigationStep]:
        ...
        current_index = (
            step_order.index(self.current_step)
...
class WorkflowActions:
    ...
    async def initial_analysis(
        context: Dict[str, Any], db: AsyncSession
    ) -> Dict[str, Any]:
        ...
        tx_query = (
            select(Transaction).where(Transaction.subject_id == subject_uuid).limit(50)
        )
        ...
        transaction_summary = {
            ...
            "avg_amount": (
                sum(float(tx.amount or 0) for tx in transactions) / len(transactions)
                if transactions
                else 0
            ),
            "date_range": {
    ...
    async def transaction_review(
        ...
        round_amounts = [
            ...
            if float(tx.amount or 0) % 100 == 0 and float(tx.amount or 0) > 500
        ]
        if round_amounts:
            red_flags.append(
                {
                    ...
                    "transactions": [
                        {"id": str(tx.id), "amount": float(tx.amount or 0)}
                        for tx in round_amounts[:5]
                    ],
                }
            )
        ...
        unusual_timing = [
            tx
            for tx in transactions
            if tx.date and (tx.date.hour < 6 or tx.date.hour > 22)
        ]
        if unusual_timing:
            red_flags.append(
                {
                    ...
                    "transactions": [
                        {"id": str(tx.id), "hour": tx.date.hour if tx.date else None}
                        for tx in unusual_timing[:5]
                    ],
                }
            )
        ...
        return {
            ...
            "recommendations": WorkflowActions._generate_transaction_recommendations(
                red_flags
            ),
        }
    ...
    async def pattern_analysis(
        ...
        return {
            ...
            "pattern_confidence": len(suspicious_patterns)
            / max(len(transactions) / 10, 1),
        }
    ...
    async def entity_verification(
        ...
        if subject and subject.encrypted_pii:
            ...
            verification_results["identity_complete"] = (
                has_name and has_address and has_id_number
            )

            if not has_address:
                verification_results["risk_factors"].append(
    ...
    async def risk_assessment(
        ...
        risk_analysis = await fraud_prevention.predict_case_escalation_risk(
            subject_id, db
        )
        ...
        for flag in red_flags:
            if flag["type"] == "high_value_transactions":
                recommendations.append(
                    f"Review {flag['count']} high-value transactions totaling ${flag['total_amount']:,.2f}"
                )
            elif flag["type"] == "round_amounts":
                recommendations.append(
                    f"Investigate {flag['count']} round-amount transactions"
                )
            elif flag["type"] == "unusual_timing":
                recommendations.append(
                    f"Verify legitimacy of {flag['count']} transactions at unusual hours"
                )

        if not recommendations:
            recommendations.append(
                "No significant red flags detected in transaction review"
            )
    ...
    def _analyze_temporal_patterns(
        dates: List[datetime], amounts: List[float]
    ) -> Dict[str, Any]:
        ...
        peak_hour = (
            max(hourly_counts.keys(), key=lambda x: hourly_counts[x])
            if hourly_counts
            else None
        )
        ...
        return {
            ...
            "total_span_days": (
                (max(sorted_dates) - min(sorted_dates)).days if sorted_dates else 0
            ),
            "transaction_frequency": (
                len(dates) / max((max(sorted_dates) - min(sorted_dates)).days, 1)
                if sorted_dates
                else 0
            ),
        }
    ...
    def _analyze_amount_patterns(amounts: List[float]) -> Dict[str, Any]:
        ...
        return {
            ...
            "coefficient_of_variation": (
                statistics.stdev(amounts) / statistics.mean(amounts)
    ...
    def _analyze_merchant_patterns(transactions: List) -> Dict[str, Any]:
        ...
        return {
            ...
            "concentration": (
                top_merchants[0][1] / total_transactions if top_merchants else 0
            ),
        }
...
def create_default_workflows() -> List[InvestigationWorkflow]:
    ...
    high_risk_workflow.add_step(
        ...
        WorkflowStep(
            InvestigationStep.INITIAL_ANALYSIS,
            "Initial Analysis",
            WorkflowActions.initial_analysis,
        ),
    )
    ...
    high_risk_workflow.add_step(
        ...
        WorkflowStep(
            InvestigationStep.TRANSACTION_REVIEW,
            "Transaction Review",
            WorkflowActions.transaction_review,
        ),
    )
    ...
    high_risk_workflow.add_step(
        ...
        WorkflowStep(
            InvestigationStep.PATTERN_ANALYSIS,
            "Pattern Analysis",
            WorkflowActions.pattern_analysis,
        ),
    )
    ...
    high_risk_workflow.add_step(
        ...
        WorkflowStep(
            InvestigationStep.ENTITY_VERIFICATION,
            "Entity Verification",
            WorkflowActions.entity_verification,
        ),
    )
    ...
    high_risk_workflow.add_step(
        ...
        WorkflowStep(
            InvestigationStep.RISK_ASSESSMENT,
            "Risk Assessment",
            WorkflowActions.risk_assessment,
        ),
    )
    ...
    high_risk_workflow.add_step(
        ...
        WorkflowStep(
            InvestigationStep.DECISION_MAKING,
            "Decision Making",
            WorkflowActions.decision_making,
        ),
    )
