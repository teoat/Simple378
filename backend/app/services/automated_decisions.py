...
class NotificationService:
    ...
    async def _send_email(
        self, notification: Dict[str, Any], recipients: List[str], db: AsyncSession
    ):
...
class DecisionTrigger:
    ...
    def __init__(
        self, workflow_engine: WorkflowEngine, notification_service: NotificationService
    ):
    ...
    def register_trigger(
        self, trigger_id: str, conditions: Dict[str, Any], actions: List[Dict[str, Any]]
    ):
    ...
    async def evaluate_triggers(
        self, subject_id: str, event_type: str, db: AsyncSession
    ):
        """
        ...
        for trigger_id, trigger_config in self.triggers.items():
            ...
            if self._matches_conditions(
                case_data, event_type, trigger_config["conditions"]
            ):
                await self._execute_actions(
                    subject_id, trigger_config["actions"], case_data, db
                )

    def _matches_conditions(
        self, case_data: Dict[str, Any], event_type: str, conditions: Dict[str, Any]
    ) -> bool:
        ...
        if (
            "risk_score_min" in conditions
            and case_data.get("risk_score", 0) < conditions["risk_score_min"]
        ):
            return False
        if (
            "risk_score_max" in conditions
            and case_data.get("risk_score", 0) > conditions["risk_score_max"]
        ):
            return False
        ...
        if (
            ...
            and case_data.get("status") not in conditions["statuses"]
        ):
            return False

        # Check amount thresholds
        if (
            "amount_min" in conditions
            and case_data.get("total_amount", 0) < conditions["amount_min"]
        ):
            return False
        if (
            "amount_max" in conditions
            and case_data.get("total_amount", 0) > conditions["amount_max"]
        ):
            return False

        # Check transaction count
        if (
            "transaction_count_min" in conditions
            and case_data.get("transaction_count", 0)
            < conditions["transaction_count_min"]
        ):
            return False

        # Check time-based conditions
        if (
            "days_active_min" in conditions
            and case_data.get("days_active", 0) < conditions["days_active_min"]
        ):
            return False
        ...
        for action in actions:
            ...
            try:
                if action_type == "update_status":
                    ...
                    await self._assign_investigator(
                        subject_id, action["investigator_id"], db
                    )

                elif action_type == "escalate":
                    await self._escalate_case(
                        subject_id, action.get("reason", "Automatic escalation"), db
                    )
                    ...
                    await self.workflow_engine.process_case_event(
                        "trigger_activated", {**case_data, "trigger_action": action}, db
                    )
    ...
    async def _update_case_status(self, subject_id: str, status: str, db: AsyncSession):
        ...
        analysis_query = select(AnalysisResult).where(
            AnalysisResult.subject_id == subject_uuid
        )
    ...
    async def _send_notification(
        self, action: Dict[str, Any], case_data: Dict[str, Any], db: AsyncSession
    ):
        """Send notification."""
        notification = {
    ...
    async def _assign_investigator(
        self, subject_id: str, investigator_id: str, db: AsyncSession
    ):
    ...
    async def _create_task(
        self, subject_id: str, action: Dict[str, Any], db: AsyncSession
    ):
...
def create_default_triggers(
    workflow_engine: WorkflowEngine, notification_service: NotificationService
) -> List[Dict[str, Any]]:
    ...
    trigger_system.register_trigger(
        "critical_risk_alert",
        {"event_type": "case_created", "risk_score_min": 90},
        [
            {"type": "escalate", "reason": "Critical risk score detected"},
            {
                ...
                "recipients": [
                    ...
                    "senior-investigator@company.com",
                ],
            },
        ],
    )
    ...
    trigger_system.register_trigger(
        ...
        {"event_type": "transaction_added", "amount_min": 50000},
        [
    ...
    trigger_system.register_trigger(
        "suspicious_pattern",
        {"event_type": "analysis_completed", "risk_score_min": 70},
        [
            {"type": "assign_investigator", "investigator_id": "pattern-specialist"},
            {
                "type": "create_task",
