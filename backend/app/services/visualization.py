...
class CashflowAnalyzer:
    ...
    async def analyze_cashflow(
        transactions: List[Transaction], db: AsyncSession
    ) -> Dict[str, Any]:
        """
        ...
        for tx in transactions:
            ...
            if amt > 0:
                ...
                if is_mirror:
                    ...
                    k in desc for k in ["external", "wire", "ach incoming", "deposit"]
                ):
        ...
        project_summary = {
            ...
            "average_transaction": (
                ...
                if expense_breakdown["project_expenses"]["transactions"] > 0
                else 0
            ),
            "top_transactions": sorted(
                project_transactions, key=lambda x: abs(x["amount"]), reverse=True
            )[:10],
        }
...
class MilestoneDetector:
    ...
    async def detect_milestones(
        ...
        # Sort by date
        sorted_txs = sorted(
            [tx for tx in transactions if tx.date], key=lambda x: x.date
        )
                ...
                clusters.append(
                    {
                        ...
                        "phase": MilestoneDetector._estimate_phase(
                            tx.date, transactions
                        ),
                    }
                )
                i = j
...
class FraudIndicatorDetector:
    ...
    async def detect_indicators(
        transactions: List[Transaction], subject_id: str
        ...
        stdev_amt = (
            statistics.stdev([abs(a) for a in amounts]) if len(amounts) > 1 else 0
        )
    ...
    def _calculate_risk_score(indicators: List[Dict[str, Any]]) -> float:
        ...
        total_score = sum(
            severity_weights.get(ind["severity"], 0) for ind in indicators
        )
