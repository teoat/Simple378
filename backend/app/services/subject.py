...
from app.db.models import Subject, Transaction, Consent
...
class SubjectService:
    ...
    async def export_subject_data(db: AsyncSession, subject_id: uuid.UUID) -> dict:
        ...
        query = select(Subject).where(Subject.id == subject_id).options(
            selectinload(Subject.transactions),
            selectinload(Subject.analysis_results).selectinload(AnalysisResult.indicators),
