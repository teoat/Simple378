# Friendly AI Meta Agent Design Orchestration

## Overview
This document outlines the design orchestration for "Friendly AI", a meta agent that serves as an intelligent, contextual assistant available at the top right of all authenticated pages (excluding authentication pages). Friendly AI provides proactive suggestions, contextual help, workflow guidance, and integrates with the existing AIAssistant for deeper conversations. It acts as the primary entry point for AI assistance throughout the application.
...
#### FriendlyAI Component (New)
- **Location:** `frontend/src/components/ai/FriendlyAI.tsx`
...
├── components/
│   └── ai/
│       ├── FriendlyAI.tsx (new - main component)
│       ├── FriendlyAIButton.tsx (new - collapsed button)
│       ├── FriendlyAIPanel.tsx (new - expanded quick view)
...
│       └── hooks/
│           └── useFriendlyAIContext.ts (new - context detection)
```
...
```typescript
// frontend/src/components/ai/hooks/useFriendlyAIContext.ts
export function useFriendlyAIContext() {
...
### ARIA Labels
- Button must have descriptive `aria-label` ("Open Friendly AI Assistant")
...
- [ ] Create `FriendlyAI.tsx` main component
- [ ] Create `FriendlyAIButton.tsx` with glassmorphism styling
...
- [ ] Create `useFriendlyAIContext.ts` hook
...
- [ ] Create `FriendlyAIPanel.tsx` component
...
- **Search Integration:** Quick search from Friendly AI panel
